document.getElementById("registerForm").addEventListener("submit", async function(event) {
    ...
    if (!birth) {
        document.getElementById("error-birthdate").textContent = "Data de nascimento é obrigatória.";
        hasError = true;
    }
    ...
    if (!privacy) {
        document.getElementById("error-submit").textContent = "Aceite a política de privacidade." ;
        hasError = true;
    }
    ...
    try {
        const response = await fetch("https://magest2api-3bbfb75c6660.herokuapp.com/lostlabcustomers", {
            method: "POST",
            headers: { "Content-Type": "application/json" },
            body: JSON.stringify({ name, email, phone, birth })
        });

        if (!response.ok) throw new Error("Erro ao enviar. Tente novamente.");
		...
		//alert(error.message);
		document.getElementById("error-submit").textContent = "Erro a submeter dados." ;
